...
// Obiekt Storage automatycznie użyje poświadczeń konta serwisowego w Cloud Run.
...
// --- Funkcje autoryzacji Google API (dostosowane do konta serwisowego) ---

/**
 * Funkcja autoryzacji dla środowiska Cloud Run, używająca konta serwisowego.
 * Automatycznie pobiera poświadczenia z środowiska Google Cloud.
 * @returns {Promise<Object>} Autoryzowany klient GoogleAuth.
 */
async function authorize() {
  const auth = new google.auth.GoogleAuth({
    // Określamy zakresy uprawnień, których aplikacja potrzebuje.
    // Te zakresy muszą być zgodne z rolami IAM nadanymi kontu serwisowemu.
    scopes: [
      'https://www.googleapis.com/auth/spreadsheets', // Do odczytu i zapisu w Arkuszach Google
      'https://www.googleapis.com/auth/gmail.send',    // Do wysyłania e-maili
      'https://www.googleapis.com/auth/devstorage.read_write' // Do odczytu i zapisu w Cloud Storage
    ],
  });
  // getClient() automatycznie pobiera poświadczenia z konta serwisowego w Cloud Run
  const client = await auth.getClient();
  console.log('Autoryzacja Google API dla Cloud Run zainicjowana za pomocą konta serwisowego.');
  return client;
}
 ...
 * @param {Object} auth - Obiekt autoryzacji Google (GoogleAuth).
...
async function runDailyTasks() {
  let auth;
  try {
    // Krok 1: Autoryzacja do Google API za pomocą konta serwisowego
    auth = await authorize(); 
    console.log('Autoryzacja Google API pomyślna!');

    // --- Konfiguracja Arkusza Google pobrana ze zmiennych środowiskowych ---
    ...
    if (!SPREADSHEET_ID || !SHEET_NAME || !RECIPIENT_EMAIL || !GCS_BUCKET_NAME || !process.env.OPENAI_API_KEY) {
      console.error('BŁĄD: Brakuje jednej lub więcej zmiennych środowiskowych (SPREADSHEET_ID, SHEET_NAME, RECIPIENT_EMAIL, GCS_BUCKET_NAME, OPENAI_API_KEY). Upewnij się, że są ustawione w konfiguracji Cloud Run.');
    ...
    await appendRowToSheet(auth, SPREADSHEET_ID, SHEET_NAME, newRowData);
    ...
    console.error('Upewnij się, że wszystkie zmienne środowiskowe są poprawnie ustawione w Cloud Run (SPREADSHEET_ID, SHEET_NAME, RECIPIENT_EMAIL, GCS_BUCKET_NAME, OPENAI_API_KEY) oraz że konta serwisowe mają odpowiednie uprawnienia.');
    throw err; 
  }
}
...
app.listen(PORT, () => {
  ...
  console.log('--- UWAGA: Aplikacja używa autoryzacji konta serwisowego. ---');
  console.log('--- Upewnij się, że konto serwisowe Cloud Run ma odpowiednie role IAM. ---');
});

// LOKALNE Uruchomienie głównej funkcji aplikacji (dla jednorazowego testu poza Express)
// Odkomentuj tylko do testów lokalnych, aby wywołać runDailyTasks bezpośrednio.
/*
runDailyTasks().catch(err => {
    console.error("Błąd podczas lokalnego uruchomienia głównej funkcji:", err);
});
*/
